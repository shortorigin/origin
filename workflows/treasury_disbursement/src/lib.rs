...
pub fn workflow_boundary() -> WorkflowBoundaryV1 {
    WorkflowBoundaryV1 {
        workflow_name: "treasury_disbursement".into(),
        touched_domains: vec!["finance_treasury".to_owned(), "hr_talent".to_owned()],
        target_services: vec!["finance-service".into(), "approval-service".into()],
        emits_evidence: true,
    ...
    E: EvidenceSink,
{
    let guarded_request = GuardedMutationRequest {
        ...
        workflow_name: "treasury_disbursement".into(),
        target_service: "finance-service".into(),
        target_aggregate: AggregateId::from("treasury_ledger"),
