...
pub const ENV_SURREALDB_ENDPOINT: &str = "SURREALDB_ENDPOINT";
pub const ENV_SURREALDB_NAMESPACE: &str = "SURREALDB_NAMESPACE";
pub const ENV_SURREALDB_DATABASE: &str = "SURREALDB_DATABASE";
pub const ENV_SURREALDB_USERNAME: &str = "SURREALDB_USERNAME";
pub const ENV_SURREALDB_PASSWORD: &str = "SURREALDB_PASSWORD";
...
pub type DurableKnowledgeStoreBackend = SurrealRepositoryContext<WsClient>;
...
impl SurrealConnectionConfig {
    ...
    pub fn from_env() -> InstitutionalResult<Self> {
        let endpoint = env_var_required(ENV_SURREALDB_ENDPOINT)?;
        let namespace = env_var_required(ENV_SURREALDB_NAMESPACE)?;
        let database = env_var_required(ENV_SURREALDB_DATABASE)?;
        let username = env::var(ENV_SURREALDB_USERNAME).ok();
        let password = env::var(ENV_SURREALDB_PASSWORD).ok();
        let config = Self {
            endpoint,
            namespace,
            database,
    ...
    pub fn validate(&self) -> InstitutionalResult<()> {
        if self.endpoint.trim().is_empty() {
            return Err(config_error("endpoint must not be empty"));
        }
        if self.namespace.trim().is_empty() {
            return Err(config_error("namespace must not be empty"));
        }
        if self.database.trim().is_empty() {
            return Err(config_error("database must not be empty"));
        }
...
) -> InstitutionalResult<SurrealRepositoryContext<WsClient>> {
    config.validate()?;
...
pub async fn connect_durable_from_env() -> InstitutionalResult<SurrealRepositoryContext<WsClient>> {
...
fn env_var_required(name: &str) -> InstitutionalResult<String> {
    env::var(name)
        .map_err(|_| config_error(format!("missing required environment variable `{name}`")))
}
...
fn config_error(message: impl Into<String>) -> InstitutionalError {
    InstitutionalError::configuration(
        OperationContext::new("shared/surrealdb-access", "validate_connection_config"),
        message,
    )
}
